...
void add_free_space_range(Project *p, const Range range)
{
    ...
    p->free_space_file_data = realloc(p->free_space_file_data, sizeof(Range) * (p->free_space_file_size + 1));
    p->free_space_file_data[p->free_space_file_size] = range;
    p->free_space_file_size++;
}
...
size_t get_function_instructions_start_offset(Function *func) {
    size_t size = 0;
    // sequantially compute attributes sizeof of bytes, include arrays
    size += sizeof(func->obj_type);
    size += sizeof(func->offset);
...
size_t serialize_Instruction(const Instruction *instr, unsigned char *buffer)
{
    ...
    memcpy(buffer + size, &(instr->i_type), sizeof(InstructionEnum));
    size += sizeof(InstructionEnum);
...
Instruction deserialize_Instruction(FILE *file)
{
    ...
    if (fread(&(instr.i_type), sizeof(InstructionEnum), 1, file) != 1)
    {
    ...
    case MOV:
    {
        ...
        // Example: deserialize_Node(file, &(mov_data->dest));
...
size_t serialize_Node(const Node *node, unsigned char *buffer)
{
    ...
    case SIZEOF:
        ...
        offset += sizeof(char);
        // Serialize structure_access array
        memcpy(buffer + offset, node->data.sizeof_.structure_access.object.struct_type, sizeof(unsigned long long));
        offset += sizeof(unsigned long long);
        break;
...
Node deserialize_Node(FILE *file)
{
    ...
    fread(&(node.type), sizeof(enum NodeType), 1, file);
    ...
    case L_REG:
        ...
        fread(node.data.reg.name, 1, name_len, file);
        node.data.reg.name[name_len] = '\0'; // Ensure null termination
        // Deserialize value
        fread(&(node.data.reg.value), sizeof(unsigned long long), 1, file);
        break;
    case C_VAR:
        // Example deserialization for Variable
        fread(&(node.data.var.offset), sizeof(unsigned long long), 1, file);
        // Deserialize other fields of Variable as needed
        break;
    case CONSTANT:
        // Example deserialization for constant (offset)
        fread(&(node.data.offset), sizeof(unsigned long long), 1, file);
        break;
    case SIZEOF:
        // Example deserialization for SizeofType
        // Deserialize isBasic flag
        fread(&(node.data.sizeof_.isBasic), sizeof(char), 1, file);
        // Deserialize structure_access array
        fread(node.data.sizeof_.structure_access.object.struct_type, sizeof(unsigned long long), 1, file);
        break;
        // Add cases for other NodeType values as needed
...
size_t serialize_variable(const Variable *var, unsigned char* buffer)
{
    ...
    memcpy(buffer + offset, &var->offset, sizeof(ull));
    offset += sizeof(ull);

    size_t name_len = strlen(var->name);
    memcpy(buffer + offset, &name_len, sizeof(size_t));
    offset += sizeof(size_t);
    memcpy(buffer + offset, var->name, name_len);
    offset += name_len;

    memcpy(buffer + offset, &var->visibility, sizeof(ObjectVisibility));
    offset += sizeof(ObjectVisibility);
    memcpy(buffer + offset, &var->vtype, sizeof(var->vtype));
    offset += sizeof(var->vtype);
    ...
    {
        //offset += serialize_struct_type(buffer + offset, var->data.struct_type);
    }
...
Function *deserialize_function(Project *p, ull offset)
{
    ...
    fread(&fun->instructions_size, sizeof(unsigned int), 1, f);

    for (int i = 0; i < fun->instructions_size; i++)
    {
        fun->instructions[i] = deserialize_Instruction(f);
    }
...
void handle_MOV(Project *p, Instruction *instr)
{
    ...
    if (dms->src.type == L_REG || dms->src.type == C_VAR)
    {
        if (dms->dest.type == L_REG || dms->dest.type == C_VAR)
        {
            ...
            {
                if (dms->src_bytes_to_move.type == CONSTANT)
                { // consider rather constant
...
calculate:
    ...
    for (size_t i = 0; i < arith->src_size; ++i)
    {
        if (arith->src[i].type == L_REG)
        {
            result += arith->src[i].data.reg.value;
            // printf("result: %llu\n", result);
            // printf("arith->src[i].data.reg.value: %llu\n", arith->src[i].data.reg.value);
        }
        else if (arith->src[i].type == C_VAR)
        {
            memcpy(&result, &arith->src[i].data.var.value, sizeof(ull));
        }
        else if (arith->src[i].type == G_VAR)
        {
            memcpy(&result, &arith->src[i].data.var.value, sizeof(ull));
        }
        else if (arith->src[i].type == SIZEOF)
        {
            if (arith->src[i].data.sizeof_.isBasic)
            {
                ull bs = get_basic_type_size(arith->src[i].data.sizeof_.basic_type);
                result += bs;
            }
            else
            {
                // is structure, we need to calculate size of structure
                ull bs = get_structure_sizeof_recursive(p, arith->src[i].data.sizeof_.structure_access.object.struct_type, 0);
                result += bs;
            }
        }
        else if (arith->src[i].type == STRUCTOFFSET)
        {
            // check if offset is variable or struct and if it is in some structure up to the top one and then calculate offset
            // ull size = get_structure_sizeof_recursive();
        }
    ...
    if (arith->dest.type == L_REG)
    {
        arith->dest.data.reg.value += result;
    }
    else if (arith->dest.type == C_VAR)
    {
        result += arith->dest.data.var.value.ulong_val;
        memcpy(&arith->dest.data.var.value, &result, sizeof(ull));
    }
    else if (arith->dest.type == G_VAR)
    {
        result += arith->dest.data.var.value.ulong_val;
        memcpy(&arith->dest.data.var.value, &result, sizeof(ull));
    }
...
incdec:
    Arithmetic_s *arith_incdec = (Arithmetic_s *)instr->data;
    // example: INC dest, src(number)

    if (arith_incdec->src[0].type == L_REG)
    {
        result = arith_incdec->src[0].data.reg.value;
    }
    else if (arith_incdec->src[0].type == C_VAR)
    {
        result += arith->dest.data.var.value.ulong_val;
        memcpy(&result, &arith_incdec->src[0].data.var.value, sizeof(ull));
    }
    else if (arith->dest.type == G_VAR)
    {
        result += arith->dest.data.var.value.ulong_val;
    ...
    else if (arith->dest.type == SIZEOF)
    {
        if (arith->dest.data.sizeof_.isBasic)
        {
            ull bs = get_basic_type_size(arith->dest.data.sizeof_.basic_type);
            result += bs;
        }
        else
        {
            // is structure, we need to calculate size of structure
            ull bs = get_structure_sizeof_recursive(p, arith->dest.data.sizeof_.structure_access.object.struct_type, 0);
            result += bs;
        }
    }
    else if (arith->dest.type == STRUCTOFFSET)
    {
        // check if offset is variable or struct and if it is in some structure up to the top one and then calculate offset
        // ull size = get_structure_sizeof_recursive();
    }
    else if (arith->src->type == CONSTANT)
    {
        result = -arith->src[0].data.offset;
    }
    return;
}
